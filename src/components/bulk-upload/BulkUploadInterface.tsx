...
import { finalizeClientUploadAction, FileUploadMeta, createBatchJobAction, completeBatchJobAction, getAgentsAction } from '@/lib/actions/bulk-upload.actions';
...
export default function BulkUploadInterface() {
  ...
  const [uploadResults, setUploadResults] = useState<{name: string, success: boolean, link?: string, error?: string}[]>([]);
  ...
  const downloadExcel = () => {
    ...
    worksheet['!cols'] = [{ wch: 30 }, { wch: 15 }, { wch: 20 }, { wch: 60 }];
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setModalConfig({ isOpen: true, title: 'Error Input', message: "Gagal membaca file Excel. Pastikan format benar dan mengandung kolom 'Nama Lengkap', 'Tanggal Lahir', & 'PIC'.", type: 'alert' });
    }
  };

  const handleValidate = () => {
    if (!excelEntries) {
      setModalConfig({ isOpen: true, title: 'Perhatian', message: "Silakan upload file Excel terlebih dahulu sebagai acuan validasi.", type: 'alert' });
      return;
    }
    
    if (dattaiFiles.length === 0 && resiFiles.length === 0 && kwitansiFiles.length === 0) {
      setModalConfig({ isOpen: true, title: 'Perhatian', message: "Silakan upload file dokumen terlebih dahulu.", type: 'alert' });
      return;
    }
  ...
  const executeUpload = () => {
    ...
    setModalConfig({
      ...
      message: 'Apakah Anda yakin ingin memulai upload? Proses ini tidak dapat dibatalkan di tengah jalan.',
      type: 'confirm',
      onConfirm: () => {
  ...
  const startUploadProcess = async () => {
    ...
    const { batchId } = await createBatchJobAction(total);

    const tasks = validationResult.valid.map((client) => limit(async () => {
      try {
        ...
        setUploadResults(prev => [...prev, { name: client.clientName, success: true, link: res.accessLink }]);
        
      } catch (err: any) {
        console.error("Failed for client:", client.clientName, err);
        setUploadResults(prev => [...prev, { name: client.clientName, success: false, error: err.message || "Unknown error" }]);
      } finally {
        completedCount++;
  ...
  return (
    <div className="max-w-6xl mx-auto font-sans bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border rounded-xl overflow-hidden shadow-sm mb-8">
        ...
        <div className="p-6">
          <p className="text-sm text-gray-500 mb-4 font-medium">Unggah file laporan Excel yang mengandung daftar nama lengkap, tanggal lahir, dan PIC yang bertanggung jawab.</p>
          <input 
            type="file" 
            accept=".xlsx, .xls, .csv"
          ...
          {excelEntries && (
            <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-100 flex gap-4">
              <div className="text-sm text-green-800">✅ <strong>{excelEntries.length} target</strong> termuat.</div>
              <div className="text-sm text-blue-800 flex items-center">
                <span>Di-assign ke {new Set(excelEntries.map(e => e.picName).filter(Boolean)).size} PIC: </span>
                <div className="flex gap-1 ml-2">
                  {Array.from(new Set(excelEntries.map(e => e.picName).filter(Boolean))).map((picName, i) => (
                    <span key={i} className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded text-xs font-semibold">{picName}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      <div className="bg-white border rounded-xl overflow-hidden shadow-sm mb-8">
        ...
        <div className="p-6">
           <p className="text-sm text-gray-500 mb-6 font-medium">
             Masukkan file dokumen sesuai kategorinya. Anda dapat memblok gambar lalu drag-drop ke input.
           </p>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="border bg-gray-50 rounded p-4">
                ...
                <input type="file" multiple accept="image/jpeg, image/jpg" 
                  onChange={(e) => setDattaiFiles(e.target.files ? Array.from(e.target.files) : [])} 
                  className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:bg-blue-50 file:text-blue-700 file:font-semibold hover:file:bg-blue-100 cursor-pointer" disabled={!excelEntries || isUploading}
                />
              ...
              <div className="border bg-gray-50 rounded p-4">
                ...
                <p className="text-xs text-gray-500 mb-3 font-mono">Format: [NomorUrut]_[Bank]_[Depan]_[Belakang]_[NoRek].jpg</p>
                <input type="file" multiple accept="image/jpeg, image/jpg" 
              ...
              <div className="border bg-gray-50 rounded p-4">
                ...
                <p className="text-xs text-gray-500 mb-3 font-mono">Format: [NomorUrut]_[Tanggal]_[Depan]_[Belakang]_[Jenis].jpg</p>
                <input type="file" multiple accept="image/jpeg, image/jpg" 
           ...
           <div className="mt-6 pt-6 border-t flex justify-end">
              <Button onClick={handleValidate} disabled={!excelEntries || isUploading} variant="outline" className="border-blue-600 text-blue-600 hover:bg-blue-50">
        ...
        <div className="border rounded-xl bg-white shadow-sm overflow-hidden mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="p-5 bg-gray-50 border-b flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-500">Sistem memetakan dokumen ke baris Excel dan PIC.</p>
            </div>
                      ...
                      {validationResult.valid.map((v, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="px-4 py-3">
                            ...
                            <div className="text-xs text-gray-500 mt-1">DOB: {v.dob.toLocaleDateString('id-ID')}</div>
                          </td>
                          <td className="px-4 py-3">
                             <div className="font-medium text-blue-700 flex items-center gap-1">👩‍💼 {v.picName}</div>
                          </td>
                          <td className="px-4 py-3 font-mono text-xs text-gray-600 break-all space-y-1">
                            <div className="bg-gray-100 px-2 py-1 rounded truncate max-w-xs">{v.files.dattai_ichijikin.name}</div>
                            <div className="bg-gray-100 px-2 py-1 rounded truncate max-w-xs">{v.files.resi_transfer.name}</div>
                            <div className="bg-gray-100 px-2 py-1 rounded truncate max-w-xs">{v.files.kwitansi.name}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <div className="border rounded-xl p-8 bg-white shadow-sm mb-8 animate-in slide-in-from-bottom-2">
          <div className="flex justify-between items-end mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500">Menyimpan file dan menghasilkan token URL secara Atomic...</p>
            </div>
          ...
          {progress === 100 && !isUploading && (
             <div className="mb-4">
                <Button onClick={downloadExcel} className="w-full bg-green-600 hover:bg-green-700 text-white flex items-center justify-center gap-2">
          ...
          {uploadResults.length > 0 && (
            <div className="mt-8">
              <h3 className="font-semibold mb-4 text-gray-800 border-b pb-2">Log Eksekusi Real-time:</h3>
              <ul className="space-y-3">
                {uploadResults.map((res, idx) => (
                  <li key={idx} className={`p-4 rounded-lg flex gap-3 items-start ${res.success ? 'bg-green-50/50 border border-green-100' : 'bg-red-50/50 border border-red-100'}`}>
                    ...
                    <div className="flex-1">
                      ...
                      {res.success ? (
                        <div className="text-sm mt-1 flex flex-col gap-1">
                          <div><span className="text-gray-600">Link Akses:</span> {res.link}</div>
                        </div>
                      ) : (
