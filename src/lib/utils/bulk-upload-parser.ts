  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const entries: ExcelEntry[] = rawJson.map((row: any) => {
          const nama = row['Nama Lengkap'] || row['nama lengkap'] || row['Nama'] || row['NAMA'] || '';
          const dob = row['Tanggal Lahir'] || row['tanggal lahir'] || row['TANGGAL_LAHIR'] || row['DOB'] || '';
...
export function parseExcelDate(dateValue: string | number | Date): Date {
  ...
  if (typeof dateValue === 'number') {
    ...
    if (strObj.length === 8) {
       ...
       if (p3 > 1800) {
           return new Date(`${p3}-${String(p2).padStart(2,'0')}-${String(p1).padStart(2,'0')}T00:00:00Z`);
       } else if (p1 > 18) {
  ...
  if (delimiterMatch) {
    ...
    if (parts.length === 3) {
        ...
        if (p3 > 1000) {
           ...
           return new Date(`${p1}-${String(p2).padStart(2,'0')}-${String(p3).padStart(2,'0')}T00:00:00Z`);
        }
    }
  }
...
export function validateBulkUploadV2(
  ...
  const sortedEntries = [...excelEntries].sort((a, b) => b.sanitizedName.length - a.sanitizedName.length);
  ...
  for (const entry of sortedEntries) {
    ...
    if (isNaN(parsedDob.getTime()) || parsedDob.getFullYear() < 1900 || parsedDob.getFullYear() > 2100) {
      invalid.push({
        clientName: entry.namaLengkap,
        status: 'invalid',
